...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    private List<StoredDialogsFolder> storedDialogsFolders = new ArrayList<>();
    private CompositeDisposable disposables = new CompositeDisposable();

    // Основное хранилище iMe
    public StorageRepository imeStorage =
    ...
    public ArrayList<TLRPC.Dialog> dialogsBotsOnly = new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsSingleChatsOnly = new ArrayList<>();
    ...
    // Диалоги соответствующие конкретной папке
    public LongSparseArray<ArrayList<TLRPC.Dialog>> chosenDialogsByFolder = new LongSparseArray<>();
    // Диалоги, которые не входят в соответствующую папку
    public LongSparseArray<ArrayList<TLRPC.Dialog>> unchosenDialogsByFolder = new LongSparseArray<>();
    ...
    public MessagesController(int num) {
        ...
        maxPinnedDialogsCount = mainPreferences.getInt("maxPinnedDialogsCount", 10);
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    if (isProxyDialog = (proxyDialog != null && proxyDialog.id == did)) {
                        ...
                        disposables.add(
                                imeStorage.deleteDialog(UserConfig.getInstance(currentAccount).getClientUserId(), did)
                                        .subscribeOn(Schedulers.io())
                                        .observeOn(AndroidSchedulers.mainThread())
                                        .subscribe(() -> Log.d("Storage", "Dialog deleted " + did),
    ...
    private Boolean dialogIsArchived(TLRPC.Dialog dialog) {
        return getDialogs(1).contains(dialog);
    }
    ...
    public boolean checkCanOpenChat(final Bundle bundle,
                                    final BaseFragment fragment, MessageObject originalMessage) {
    ...
    public static void openChatOrProfileWith(TLRPC.User user, TLRPC.Chat
            chat, BaseFragment fragment, int type, boolean closeLast) {
    ...
    public void openByUserName(String username, final BaseFragment fragment,
                               final int type) {
    ...
    public void startDialogFoldersObserving(long userId) {
        ...
        disposables.add(
                imeStorage.streamAvailableFolders(userId)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(folders -> {
