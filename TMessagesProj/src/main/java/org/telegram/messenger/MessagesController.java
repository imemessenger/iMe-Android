...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    // iMe - start
    // Сохраненный в бд список папок
    public List<StoredDialogsFolder> storedDialogsFolders = new ArrayList<>();
    private CompositeDisposable disposables = new CompositeDisposable();

    // Основное хранилище iMe
    public StorageRepository aigramStorage =
            StorageRepository.Companion.getInstance(ApplicationLoader.applicationContext);
    // iMe - end
    ...
    // iMe - start
    public ArrayList<TLRPC.Dialog> dialogsUnread = new ArrayList<>();           // непрочитанные
    public ArrayList<TLRPC.Dialog> dialogsUndelivered = new ArrayList<>();      // недоставленные
    public ArrayList<TLRPC.Dialog> dialogsBotsOnly = new ArrayList<>();         // боты
    public ArrayList<TLRPC.Dialog> dialogsSingleChatsOnly = new ArrayList<>();  // чаты
    public ArrayList<TLRPC.Dialog> dialogsSecretChats = new ArrayList<>();  // секретные чаты
    public ArrayList<DialogsFolder> dialogFoldersAll = new ArrayList<>();       // Текущий список папок с диалогами
    public LongSparseArray<ArrayList<TLRPC.Dialog>> chosenDialogsByFolder = new LongSparseArray<>();    // Диалоги соответствующие конкретной папке
    public LongSparseArray<ArrayList<TLRPC.Dialog>> unchosenDialogsByFolder = new LongSparseArray<>();  // Диалоги, которые не входят в соответствующую папку
    // iMe - end
    ...
    // iMe
    private boolean markAllAsReadInProgress = false;
    ...
    public static SharedPreferences getBackupSettings() {
        return getInstance(0).backupPreferences;
    }
    ...
    public MessagesController(int num) {
        ...
        if (currentAccount == 0) {
            ...
            backupPreferences = ApplicationLoader.applicationContext.getSharedPreferences("backupconfig", Activity.MODE_PRIVATE);
            ...
            emojiPreferences = ApplicationLoader.applicationContext.getSharedPreferences("emoji" + currentAccount, Activity.MODE_PRIVATE);backupPreferences = ApplicationLoader.applicationContext.getSharedPreferences("backupconfig" + currentAccount, Activity.MODE_PRIVATE);
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == NotificationCenter.FileDidUpload) {
            ...
            if (dialog != null && dialog.top_message == msgId) {
                ...
                getNotificationCenter().postNotificationName(NotificationCenter.refreshTabContent); // iMe
                getNotificationCenter().postNotificationName(NotificationCenter.refreshTabIcons);   // iMe
            }
    ...
    public void cleanup() {
        ...
        // iMe - start
        dialogsUnread.clear();
        dialogsUndelivered.clear();
        dialogsBotsOnly.clear();
        dialogsSingleChatsOnly.clear();
        dialogsSecretChats.clear();
        dialogFoldersAll.clear();
        chosenDialogsByFolder.clear();
        unchosenDialogsByFolder.clear();
        // iMe - end
    ...
    public ArrayList<TLRPC.Dialog> getAllDialogsNoArchived() {
        ArrayList<TLRPC.Dialog> targetDialogs = getAllDialogsNoFolders();
        ArrayList<TLRPC.Dialog> allDialogsNoFolders = new ArrayList<>();
        for (int i = 0; i < targetDialogs.size(); i++) {
            TLRPC.Dialog dialog = targetDialogs.get(i);
            if (!isDialogArchived(dialog)) {
                allDialogsNoFolders.add(dialog);
            }
        }

        return allDialogsNoFolders;
    }

    public ArrayList<TLRPC.Dialog> getAllDialogsNoFolders() {
        ArrayList<TLRPC.Dialog> allDialogsNoFolders = new ArrayList<>();
        for (int i = 0; i < allDialogs.size(); i++) {
            TLRPC.Dialog dialog = allDialogs.get(i);
            if (!isDialogInstanceOfFolder(dialog)) {
                allDialogsNoFolders.add(dialog);
            }
        }
    ...
    private void removeDialog(TLRPC.Dialog dialog) {
        ...
        dialogsUndelivered.remove(dialog);
        dialogsBotsOnly.remove(dialog);
        dialogsSingleChatsOnly.remove(dialog);
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    if (isProxyDialog = (proxyDialog != null && proxyDialog.id == did)) {
                        ...
                        // при удалении диалога проходим по всем имеющимся папкам и удаляем диалог из каждой
                        disposables.add(
                                aigramStorage.deleteDialog(UserConfig.getInstance(currentAccount).getClientUserId(), did)
                                        .subscribeOn(Schedulers.io())
                                        .observeOn(AndroidSchedulers.mainThread())
                                        .subscribe(() -> {
                                                    NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.saveIMeBackup);
                                                    Log.d(" Storage", "Dialog deleted " + did);
                                                },
    ...
    public int addDialogToFolder(ArrayList<Long> dialogIds, int folderId, int pinnedNum, ArrayList<TLRPC.TL_inputFolderPeer> peers, long taskId) {
        ...
        // удаляем из всех папок если диалоги отправляются в архив
        if (folderId == 1) {
            disposables.add(
                aigramStorage.deleteDialogs(UserConfig.getInstance(currentAccount).getClientUserId(), dialogIds)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(() -> {
                            NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.saveIMeBackup);
                            Log.d(" Storage", "Dialogs delete: " + dialogIds.size());
                        },
                        Throwable::printStackTrace)
            );
    ...
    public void processDialogsUpdateRead(final LongSparseArray<Integer> dialogsToUpdate, final LongSparseArray<Integer> dialogsMentionsToUpdate) {
        AndroidUtilities.runOnUIThread(() -> {
            ...
            getNotificationCenter().postNotificationName(NotificationCenter.refreshTabContent);
            getNotificationCenter().postNotificationName(NotificationCenter.refreshTabIcons);
    ...
    public void performLogout(int type) {
        ...
        getBackupSettings().edit().clear().apply();
    }
    ...
    public void sortDialogs(SparseArray<TLRPC.Chat> chatsDict) {
        ...
        for (int a = 0, N = allDialogs.size(); a < N; a++) {
            ...
            int lower_id = (int) d.id;
            // iMe - start
            if (DialogObject.isSecretDialogId(d.id)) {
                dialogsSecretChats.add(d);
                dialogsSingleChatsOnly.add(d);
            }
            TLRPC.User user = MessagesController.getInstance(currentAccount).getUser(lower_id);
            if (user != null && !isDialogArchived(d)) {
                if (user.bot) {
                    dialogsBotsOnly.add(d);
                } else {
                    dialogsSingleChatsOnly.add(d);
                }
            }
            ...
            } else if (d instanceof TLRPC.TL_dialogFolder) {
                dialogsSingleChatsOnly.add(d);
                dialogsSecretChats.add(d);
                dialogsGroupsOnly.add(d);
                dialogsChannelsOnly.add(d);
    ...
    // iMe - start
    // Обновление непрочитанных, отвечает за контент соотвествующей вкладки
    public void refreshUnreadDialogs() {
        dialogsUnread.clear();
        for (int a = 0; a < allDialogs.size(); a++) {
            TLRPC.Dialog dialog = allDialogs.get(a);
            if (((dialog.unread_count != 0 || dialog.unread_mark) && !isDialogArchived(dialog)) || isDialogInstanceOfFolder(dialog)) {
                dialogsUnread.add(dialog);
            }
        }
    }

    public void refreshUndeliveredDialogs() {
        dialogsUndelivered.clear();
        for (int a = 0; a < allDialogs.size(); a++) {
            TLRPC.Dialog d = allDialogs.get(a);
            int high_id = (int) (d.id >> 32);
            int lower_id = (int) d.id;
            TLRPC.User user = MessagesController.getInstance(currentAccount).getUser(lower_id);
            MessageObject topMessage = dialogMessage.get(d.id);
            if (user != null && !user.bot && topMessage != null && topMessage.isFromUser() && topMessage.isUnread() && d.unread_count == 0) {
                dialogsUndelivered.add(d);
            }
        }
    }

    // Мои каналы
    public List<TLRPC.Dialog> getMyChannels() {
        List<TLRPC.Dialog> myChannels = new ArrayList<>();
        for (int a = 0, N = allDialogs.size(); a < N; a++) {
            TLRPC.Dialog d = allDialogs.get(a);
            int high_id = (int) (d.id >> 32);
            int lower_id = (int) d.id;
    ...
    // Где я админ
    public ArrayList<TLRPC.Dialog> getMyAdminDialogs() {
        ArrayList<TLRPC.Dialog> adminDialogs = new ArrayList<>();
        for (int a = 0, N = allDialogs.size(); a < N; a++) {
            TLRPC.Dialog d = allDialogs.get(a);
            int lower_id = (int) d.id;
            if (d instanceof TLRPC.TL_dialog) {
                TLRPC.Chat chat = getChat(-lower_id);
                if (chat != null && !chat.creator && chat.admin_rights != null) {
                    adminDialogs.add(d);
                }
            }
        }
    ...
    // Где я владелец
    public ArrayList<TLRPC.Dialog> getMyOwnerDialogs() {
        ArrayList<TLRPC.Dialog> ownerDialogs = new ArrayList<>();
        for (int a = 0, N = allDialogs.size(); a < N; a++) {
            TLRPC.Dialog d = allDialogs.get(a);
            int high_id = (int) (d.id >> 32);
            int lower_id = (int) d.id;
    ...
    // Заарзхивирован ли диалог
    public static Boolean isDialogArchived(TLRPC.Dialog dialog) {
        return dialog.folder_id == 1;
    }

    // Является ли диалог архивом
    public static Boolean isDialogInstanceOfFolder(TLRPC.Dialog dialog) {
        return dialog instanceof TLRPC.TL_dialogFolder;
    }

    // Функционал кнопки "Прочитать все"
    public void markAllDialogsAsRead() {
        markAllAsReadInProgress = true;
        for (int a = 0; a < dialogsUnread.size(); a++) {
            TLRPC.Dialog dialog = dialogsUnread.get(a);
            MessagesController.getInstance(currentAccount).markDialogAsRead(dialog.id, dialog.top_message, dialog.top_message, dialog.last_message_date, false, 0, true, 0);
        }
        markAllAsReadInProgress = false;
        getNotificationCenter().postNotificationName(NotificationCenter.dialogsNeedReload);
    }

    // Обновление содержимого папок с диалогами
    public void refreshFolderDialogs() {
        chosenDialogsByFolder.clear();
        unchosenDialogsByFolder.clear();
        ArrayList<DialogsFolder> sorted = new ArrayList<>(storedDialogsFolders.size());
        for (StoredDialogsFolder folder : storedDialogsFolders) {
            int folderUnreads = 0;
            int folderMentions = 0;
            ArrayList<TLRPC.Dialog> dialogsChosen = new ArrayList<>();
            ArrayList<TLRPC.Dialog> dialogsLeft = new ArrayList<>();
            Set<Long> currentFolderIds = folder.getIds();
            ArrayList<TLRPC.Dialog> currentDialogs = getAllDialogsNoFolders();
            for (int a = 0; a < currentDialogs.size(); a++) {
                TLRPC.Dialog dialog = currentDialogs.get(a);
                if (dialog.folder_id != 0) {
                    continue;
                }
                if (currentFolderIds.contains(dialog.id)) {
                    dialogsChosen.add(dialog);
                    folderUnreads += dialog.unread_count;
                    folderMentions += dialog.unread_mentions_count;
                } else {
                    dialogsLeft.add(dialog);
                }
            }
            if (!dialogsChosen.isEmpty()) {
                ...
                sorted.add(
                        new DialogsFolder(
                                ...
                                folderUnreads,
                                folderMentions,
                                folder.getBackgroundId(),
                                dialogsChosen
                        ));
                chosenDialogsByFolder.put(folderId, dialogsChosen);
                unchosenDialogsByFolder.put(folderId, dialogsLeft);
            }
        }
        dialogFoldersAll.clear();
        Collections.sort(sorted, (o1, o2) ->
                Integer.compare(o2.getTopDialog().last_message_date, o1.getTopDialog().last_message_date));
        Collections.sort(sorted, (o1, o2) ->
                Boolean.compare(o2.isPinned(), o1.isPinned()));
        dialogFoldersAll = sorted;
    }

    public UserConfig getUserConfig() {
        return super.getUserConfig();
    }

    public ConnectionsManager getConnectionsManager() {
        return super.getConnectionsManager();
    }

    // Подписка на обновление папок в бд
    public void startDialogFoldersObserving(long userId) {
        Log.d(" Storage", "Folders observer started for user " + userId);
        disposables.add(
                aigramStorage.streamAvailableFolders(userId)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(folders -> {
                            storedDialogsFolders = folders;
                            getNotificationCenter().postNotificationName(NotificationCenter.refreshTabContent);
                            getNotificationCenter().postNotificationName(NotificationCenter.refreshTabIcons);
                        }, Throwable::printStackTrace)
        );
    ...
    // Отписка от обновления папок
    public void stopDialogFoldersObserving() {
        Log.d(" Storage", "Folders observer stopped");
        disposables.clear();
    }
}
